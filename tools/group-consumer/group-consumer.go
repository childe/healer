...
func init() {
	...
	flag.IntVar(&consumerConfig.AutoCommitIntervalMS, "metadata.max.age.ms", consumerConfig.AutoCommitIntervalMS, "The frequency in ms that the consumer offsets are committed")
	flag.IntVar(&consumerConfig.MetadataMaxAgeMS, "auto.commit.interval.ms", consumerConfig.MetadataMaxAgeMS, "The period of time in milliseconds after which we force a refresh of metadata even if we haven't seen any partition leadership changes to proactively discover any new brokers or partitions.")
